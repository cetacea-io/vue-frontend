...
export default {
  async asyncData ({store, error, app, params}) {
    ...
    await store.dispatch('courses/get_courses', 'teatro')
    await store.dispatch('courses/get_courses', 'general')
  ...
  methods: {
    add(){
      this.$store.dispatch('courses/get_courses', 'teatro')
      this.$store.dispatch('courses/get_courses', 'general')
    },
    // ...mapMutations({
      // getMoreCourses: 'courses/getmore'
    // })
  }
}
</script>
